...
let FirebasePerformance: FirebasePerformancePlugin | null = null;

const _initPromise = import('@capacitor/core').then((cap) => {
  Capacitor = cap.Capacitor;
  if (Capacitor?.isNativePlatform()) {
    FirebasePerformance = cap.registerPlugin<FirebasePerformancePlugin>('FirebasePerformancePlugin');
  }
}).catch(() => {
...
class MonitoringService {
  ...
  async trace<T>(traceName: string, action: () => Promise<T>): Promise<T> {
    if (!Capacitor?.isNativePlatform() || !FirebasePerformance) {
