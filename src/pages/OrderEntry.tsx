...
export const OrderEntry = () => {
  ...
  */
  ...
  const addToCart = async (product: Product) => {
    ...
    // STRICT UOM-Master-driven conversion. The dropdown only ever offers
    // codes that exist in product_uom_mapping (filtered by enabled UOM Master).
    // No hardcoded kg/grams math, no silent fallbacks.
    let effectiveRate = Number(displayProduct.rate);
    const selectedUnit = selectedUnits[product.id] || '';
    let resolvedBaseUnit = '';
    ...
    try {
      ...
      if (units.length === 0) {
        toast({
          ...
          description: "This product has no units defined. Configure units in Product Master first.",
          variant: "destructive",
        });
        return;
      }
      if (!selectedUnit) {
        toast({
          title: "Select a unit",
          description: "Choose a unit before adding the item.",
          variant: "destructive",
        });
        return;
      }
      const target = units.find((u) => u.code.toUpperCase() === selectedUnit.toUpperCase());
      const basis = units.find((u) => u.isPriceBasis) ?? units.find((u) => u.isBase);
      const baseRow = units.find((u) => u.isBase);
      resolvedBaseUnit = baseRow?.code ?? '';
      if (!target || !basis || !basis.conversionToBase) {
        toast({
          title: "Unit not available",
          description: `The selected unit "${selectedUnit}" is not configured for this product.`,
          variant: "destructive",
        });
        return;
      }
      effectiveRate = Number(displayProduct.rate) * (target.conversionToBase / basis.conversionToBase);
    } catch (e) {
      console.error('[OrderEntry] UOM conversion failed', e);
      toast({
        title: "Unit conversion failed",
        description: "Could not load unit configuration. Please try again.",
        variant: "destructive",
      });
      return;
    }
    ...
    const cartItem = {
      ...
      unit: selectedUnit,
      // Base unit from UOM Master mapping — single source of truth
                                 ...
                                 <div className="text-xs flex items-center gap-1">
                                   ...
                                   <div>
                                     <UnitSelect
                                       productId={product.id}
                                       baseRate={variantPrice}
                                       value={selectedUnits[variantCompositeId]}
                                       onChange={(value) =>
