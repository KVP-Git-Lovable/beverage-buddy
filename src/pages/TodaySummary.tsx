...
import { Download, Share, FileText, Clock, MapPin, CalendarIcon, ExternalLink, Users, X, CreditCard, Wallet, Banknote, ShoppingCart } from "lucide-react";
...
export const TodaySummary = () => {
  ...
  // Order source breakdown data
  const [orderSourceBreakdown, setOrderSourceBreakdown] = useState<Array<{
    source: string;
    label: string;
    amount: number;
    count: number;
  ...
  const fetchTodaysData = async (isBackgroundRefresh = false) => {
    try {
      ...
      // Calculate order source breakdown
      const orderSourceMap = new Map<string, { amount: number; count: number }>();
      const orderSourceColors: Record<string, string> = {
        'store_visit': 'hsl(142, 76%, 36%)',    // green
        'portal_order': 'hsl(280, 67%, 50%)',    // purple
        'voice_assistant': 'hsl(220, 90%, 56%)', // blue
      };
      const orderSourceLabels: Record<string, string> = {
        'store_visit': 'Store Visit',
        'portal_order': 'Portal Order',
        'voice_assistant': 'Phone Order',
      };
      todayOrders?.forEach(order => {
        const source = order.order_source || 'store_visit';
        const normalizedSource = source === 'portal_order' || source === 'voice_assistant' ? source : 'store_visit';
        const existing = orderSourceMap.get(normalizedSource) || { amount: 0, count: 0 };
        orderSourceMap.set(normalizedSource, {
          amount: existing.amount + Number(order.total_amount ?? 0),
          count: existing.count + 1,
        });
      });
      const orderSourceData = Array.from(orderSourceMap.entries())
        .filter(([_, data]) => data.count > 0)
        .map(([source, data]) => ({
          source,
          label: orderSourceLabels[source] || source,
          amount: Math.round(data.amount),
          count: data.count,
          color: orderSourceColors[source] || 'hsl(220, 14%, 50%)',
        }))
        .sort((a, b) => b.amount - a.amount);
      setOrderSourceBreakdown(orderSourceData);
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      // Order by Source - before Payment Method
      if (orderSourceBreakdown.length > 0) {
        if (yPosition > 230) {
          doc.addPage();
          yPosition = 20;
        }
        doc.setFontSize(14);
        doc.setTextColor(60, 60, 60);
        doc.text("Order by Source", 14, yPosition);
        yPosition += 8;
        
        const totalSourceAmount = orderSourceBreakdown.reduce((sum, s) => sum + s.amount, 0);
        const sourceData = orderSourceBreakdown.map(s => {
          const percent = totalSourceAmount > 0 ? ((s.amount / totalSourceAmount) * 100).toFixed(0) + '%' : '0%';
          return [s.label, `${s.count} orders`, `Rs. ${s.amount.toLocaleString('en-IN')}`, percent];
        });
      ...
      // Payment Method Breakdown in PDF
      if (paymentMethodBreakdown.length > 0) {
        ...
        const totalPayment = paymentMethodBreakdown.reduce((sum, p) => sum + p.amount, 0);
        const paymentData = paymentMethodBreakdown.map(p => {
          const percent = totalPayment > 0 ? ((p.amount / totalPayment) * 100).toFixed(0) + '%' : '0%';
          return [p.method, `${p.count} orders`, `Rs. ${p.amount.toLocaleString('en-IN')}`, percent];
        });
        ...
        {orderSourceBreakdown.length > 0 && (
          <Card className="shadow-card">
            ...
            <CardContent className="space-y-2">
              {orderSourceBreakdown.map((item, index) => {
                const total = orderSourceBreakdown.reduce((sum, s) => sum + s.amount, 0);
