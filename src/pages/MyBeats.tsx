      ...
      if (userCachedBeats.length > 0) {
        ...
        const cachedRetailersData = cachedRetailers.filter((r: any) => 
          effectiveUserIds.includes(r.user_id) && (
            ...
            (r.beat_name && r.beat_name !== '')
          )
        );
        
        const retailerCountByBeatId = new Map<string, number>();
        const retailerCountByBeatName = new Map<string, number>();
        cachedRetailersData.forEach((item: any) => {
          if (item.beat_id && item.beat_id !== '' && item.beat_id !== 'unassigned') {
            retailerCountByBeatId.set(item.beat_id, (retailerCountByBeatId.get(item.beat_id) || 0) + 1);
          }
          if (item.beat_name && item.beat_name !== '') {
            retailerCountByBeatName.set(item.beat_name, (retailerCountByBeatName.get(item.beat_name) || 0) + 1);
          }
        });
        
        const beatsArray = userCachedBeats.map((beat: any, index) => {
          const count = retailerCountByBeatId.get(beat.beat_id) || retailerCountByBeatName.get(beat.beat_name) || 0;
          return {
          id: beat.beat_id,
          name: beat.beat_name,
        ...
        try {
          ...
          if (!beatsError && onlineBeats) {
            ...
            if (!retailersError && onlineRetailers) {
              const retailersData = (onlineRetailers || []).filter((r: any) => 
                (r.beat_id && r.beat_id !== '' && r.beat_id !== 'unassigned') || 
                (r.beat_name && r.beat_name !== '')
              );
              
              // Build count maps by both beat_id and beat_name for fallback matching
              const retailerCountByBeatId = new Map<string, number>();
              const retailerCountByBeatName = new Map<string, number>();
              ...
              const beatsArray = beatsData.map((beat: any, index) => {
                // Try beat_id match first, fall back to beat_name match
                const count = retailerCountByBeatId.get(beat.beat_id) || retailerCountByBeatName.get(beat.beat_name) || 0;
                return {
                id: beat.beat_id,
                name: beat.beat_name,
