  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      await monitoring.trace('distributor_login_process', async () => {
        ...
        // Reject pure Field Sales users from Distributor Portal.
        // 1) Try get_user_type RPC. 2) If RPC errors, manual fallback.
        const rejectAsFieldSales = async () => {
          await supabase.auth.signOut();
          toast.error('This account is for the Field Sales App. Please log in at /auth', {
            action: {
              label: 'Go to Field Sales Login',
              onClick: () => navigate('/auth'),
            },
            duration: 8000,
          });
          throw new Error('Field Sales account cannot access Distributor Portal');
        };

        const { data: userType, error: userTypeError } = await supabase
          .rpc('get_user_type', { p_user_id: authData.user.id });
        ...
        if (userTypeError) {
          ...
          // Pure field-sales user (has profile, no distributor row) → reject
          if (!isDistributor && hasFieldProfile) {
            await rejectAsFieldSales();
          }
        } else if (userType === 'field_sales') {
          await rejectAsFieldSales();
        }
