...
import { getDisplayValues, formatDisplayQty } from '@/utils/unitDisplayUtils';
...
const PackingList = () => {
  ...
  const loadPackingData = async (distributorId: string, dateOption: string) => {
    try {
      ...
      let allOrdersWithItems: OrderWithRetailer[] = [];

      // ---- 1. Fetch PRIMARY orders (B2D) targeting this distributor ----
      try {
        // Determine if this distributor is OEM (top of hierarchy)
        ...
        if (isOEM) {
          primaryQuery = primaryQuery.is('target_distributor_id', null);
        } else {
          primaryQuery = primaryQuery.eq('target_distributor_id', distributorId);
        }

        // Also fetch orders from child distributors
        const { data: childDistributors } = await supabase
          .from('distributors')
          .select('id, name')
          .eq('parent_id', distributorId);

        const childIds = childDistributors?.map(c => c.id) || [];

        const { data: primaryOrders } = await primaryQuery;

        // Also get orders where source is a child distributor
        let childPrimaryOrders: any[] = [];
        if (childIds.length > 0) {
          const { data: childOrders } = await supabase
            .from('primary_orders')
            .select(`
              id, order_number, order_date, expected_delivery_date, status,
              total_amount, source_distributor_id, target_distributor_id, created_at,
              primary_order_items(id, product_id, product_name, quantity, unit, unit_price)
            `)
            .is('packing_list_id', null)
            .in('status', ['confirmed', 'processing', 'submitted'])
            .in('source_distributor_id', childIds)
            .gte('created_at', start.toISOString())
            .lte('created_at', end.toISOString())
            .order('created_at', { ascending: false });
          childPrimaryOrders = childOrders || [];
        }

        // Merge and deduplicate primary orders
        const allPrimary = [...(primaryOrders || []), ...childPrimaryOrders];
        const uniquePrimary = Array.from(new Map(allPrimary.map(o => [o.id, o])).values());

        // Fetch source distributor names
        const sourceIds = [...new Set(uniquePrimary.map((o: any) => o.source_distributor_id).filter(Boolean))];
        ...
        if (sourceIds.length > 0) {
          const { data: sources } = await supabase
            ...
            .in('id', sourceIds);
          if (sources) {
            sourceMap = sources.reduce((acc, s) => { acc[s.id] = s.name; return acc; }, {} as Record<string, string>);
          }
        }

        // Process primary orders into packing items and orders list
        uniquePrimary.forEach((order: any) => {
          const sourceName = sourceMap[order.source_distributor_id] || 'Unknown Distributor';
          const items = order.primary_order_items || [];

          items.forEach((item: any) => {
            const key = `${item.product_id}-${item.unit || 'units'}`;
            if (productMap.has(key)) {
              const existing = productMap.get(key)!;
              existing.total_quantity += item.quantity;
              existing.order_count += 1;
              existing.retailers.push({ name: sourceName, quantity: item.quantity });
            } else {
              productMap.set(key, {
                ...
                order_count: 1,
                retailers: [{ name: sourceName, quantity: item.quantity }],
              });
            }
          });
        ...
        console.error('Error fetching primary orders:', primaryError);
      }

      // ---- 2. Fetch SECONDARY orders (B2R) ----
      ...
      if (retailers && retailers.length > 0) {
        ...
        if (ordersData && ordersData.length > 0) {
          ...
          itemsData?.forEach(item => {
            ...
            if (!order) return;
            const { displayQty, displayUnit } = getDisplayValues(item.quantity, 0, 0, item.unit || 'piece');
            const key = `${item.product_id}-${displayUnit}`;
            if (productMap.has(key)) {
              const existing = productMap.get(key)!;
              existing.total_quantity += displayQty;
              existing.order_count += 1;
              existing.retailers.push({ name: order.retailer_name, quantity: displayQty });
            } else {
              productMap.set(key, {
                product_id: item.product_id,
                product_name: item.product_name,
                ...
                order_count: 1,
                retailers: [{ name: order.retailer_name, quantity: displayQty }],
              });
            }
          });
                    ...
                    {order.items.map((item, idx) => (
                      <div key={idx} className="flex justify-between text-sm">
                        ...
                        <span className="text-muted-foreground">
                          {(() => {
                            const { displayQty, displayUnit } = getDisplayValues(item.quantity, 0, 0, item.unit || 'piece');
                            return `${formatDisplayQty(displayQty)} ${displayUnit}`;
                          })()}
                        </span>
                      </div>
                    ))}
