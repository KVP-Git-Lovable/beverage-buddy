...
const DistributorInventory = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const invResult = await supabase
        ...
        .select('id, product_id, product_name, quantity, reserved_quantity, damaged_quantity, expired_quantity, updated_at, warehouse_id, unit')
      ...
      // Load transactions separately (use balance_qty — the legacy "quantity" column was removed)
      const txnResult = await supabase
        .from('distributor_inventory_transactions')
        .select('id, reference_type, transaction_type, balance_qty, running_balance, notes, created_at, product_name, product_id, batch_number, unit')
        .eq('distributor_id', distributorId)
        .order('created_at', { ascending: false })
        .limit(10);
      // Map balance_qty -> quantity so downstream display components keep working
      setTransactions((txnResult.data || []).map((t: any) => ({ ...t, quantity: t.balance_qty })));
  ...
  // Raw summaryData kept for potential non-display uses
  // Display-converted values computed below after productUnitMap is built
  ...
  // Convert summary values to display units using each inventory row's own unit
  const summaryDisplayData = (() => {
    let available = 0, reserved = 0, damaged = 0, expired = 0;
    filteredInventory.forEach(i => {
      const rowUnit = i.unit || '';
      available += getDisplayValues(i.available_quantity || 0, 1, 1, rowUnit).displayQty;
      reserved += getDisplayValues(i.reserved_quantity || 0, 1, 1, rowUnit).displayQty;
      damaged += getDisplayValues(i.damaged_quantity || 0, 1, 1, rowUnit).displayQty;
      expired += getDisplayValues(i.expired_quantity || 0, 1, 1, rowUnit).displayQty;
    });
