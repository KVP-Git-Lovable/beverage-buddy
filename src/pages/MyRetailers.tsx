...
export const MyRetailers = () => {
  ...
  // Track which retailers were converted from external DB ("Purchased Retailer")
  const [purchasedRetailerIds, setPurchasedRetailerIds] = useState<Set<string>>(new Set());
  ...
  // Fetch which retailers were converted from external DB
  useEffect(() => {
    ...
    const fetchPurchased = async () => {
      ...
      for (let i = 0; i < ids.length; i += 100) {
        ...
        const { data } = await (supabase as any)
          ...
          .select('converted_retailer_id')
          .in('converted_retailer_id', batch)
          .not('converted_retailer_id', 'is', null);
        if (data) {
          allConverted.push(...data.map((d: any) => d.converted_retailer_id));
        }
      }
      setPurchasedRetailerIds(new Set(allConverted));
    };
    fetchPurchased();
  }, [retailers]);
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          >
                            ...
                            {purchasedRetailerIds.has(r.id) && (
                              <span className="text-[10px] text-blue-600 border border-blue-600 rounded px-1.5 py-0.5 whitespace-nowrap">
                                Purchased Retailer
                              </span>
                            )}
                          </h3>
