...
export const MyVisits = () => {
  ...
  const retailers = useMemo(() => {
    ...
    return optimizedRetailers.map(retailer => {
      ...
      // Determine dominant order source
      const orderSource = orders.find(o => o.order_source === 'portal_order')?.order_source 
        || orders.find(o => o.order_source === 'voice_assistant')?.order_source 
        || null;
  ...
  const filteredVisits = useMemo(() => {
    ...
    const filtered = allVisits.filter(visit => {
      // Search by name, phone, or order source label
      const searchLower = searchTerm.toLowerCase();
      const orderSourceLabel = (visit as any).orderSource === 'portal_order' ? 'portal order' 
        : (visit as any).orderSource === 'voice_assistant' ? 'phone order' 
        : (visit as any).hasOrder ? 'store visit' : '';
      const matchesSearch = visit.retailerName.toLowerCase().includes(searchLower) 
        || visit.phone.includes(searchTerm)
        || orderSourceLabel.includes(searchLower);
