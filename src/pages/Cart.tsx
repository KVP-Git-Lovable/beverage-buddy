...
export const Cart = () => {
  ...
  const [gstByProduct, setGstByProduct] = React.useState<Record<string, number>>({});
  ...
  // Load GST % from product master for items in cart
  React.useEffect(() => {
    const ids = Array.from(new Set(cartItems.map(i => i.id).filter(Boolean)));
    if (ids.length === 0) {
      setGstByProduct({});
      return;
    }
    const missing = ids.filter(id => gstByProduct[id] === undefined);
    ...
    (async () => {
      try {
        ...
        if (!data) return;
        setGstByProduct(prev => {
          const next = { ...prev };
          for (const p of data as any[]) {
            next[p.id] = Number(p.gst_percentage ?? 0) || 0;
          }
          return next;
        });
      } catch (err) {
        console.warn('[Cart] Failed to load product GST map:', err);
      }
    })();
  }, [cartItems, gstByProduct]);

  const getProductGstPct = (item: AnyCartItem): number => {
    if (!item || !item.id) return 0;
    return gstByProduct[item.id] ?? 0;
  };
  ...
  // Per-item GST sourced from product master, split equally into CGST + SGST.
  const getTotalGST = () => {
    try {
      return cartItems.reduce((sum, item) => {
        if (!item) return sum;
        const lineSubtotal = computeItemSubtotal(item);
        const lineDiscount = computeItemDiscount(item);
        const taxable = Math.max(0, lineSubtotal - lineDiscount);
        const gstPct = getProductGstPct(item);
        return sum + taxable * (gstPct / 100);
      }, 0);
    } catch (error) {
      console.error('Error computing GST:', error);
      return 0;
    }
  };
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      // Direct orders with full payment are delivered on the spot —
      // mark them as delivered with full collection
      const isDirectPaidOrder = !isCreditOrder && paymentType === 'full';
      ...
      const orderData = {
        ...
        status: isDirectPaidOrder ? 'delivered' : 'confirmed',
        delivery_status: isDirectPaidOrder ? 'delivered' : undefined,
        amount_collected: isDirectPaidOrder ? totalAmount : (creditPaid || 0),
        payment_status: isDirectPaidOrder ? 'paid' : (isCreditOrder ? 'pending' : (paymentType === 'partial' ? 'partial' : 'pending')),
      ...
      const orderItems = cartItems.map(item => {
        ...
        // Per-item GST sourced from product master (split equally CGST + SGST)
        const gstPct = getProductGstPct(item);
        const lineTax = itemTotal * (gstPct / 100);
            ...
            <Card>
              <CardContent className="p-3 space-y-2">
                ...
                <div className="border-t pt-2 space-y-1">
                  {(() => {
                    const rates = Array.from(new Set(cartItems.map(i => getProductGstPct(i))));
                    const uniformRate = rates.length === 1 ? rates[0] : null;
                    const rateLabel = uniformRate != null ? ` (${(uniformRate / 2).toFixed(2)}%)` : '';
                    return (
                      <>
                ...
                {/* Payment Type Selection - Always show Full / Partial / Credit */}
                ...
                {/* Payment Method Selection - Shown whenever full or partial payment is selected */}
                ...
                {/* Submit Order Button - Standard Full / Partial / Credit flow */}
                <Button 
