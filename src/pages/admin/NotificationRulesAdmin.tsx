...
import { ArrowLeft, Plus, Pencil, Trash2, Bell, Send, Megaphone } from 'lucide-react';
...
const NotificationRulesAdmin = () => {
  ...
  const pushMutation = useMutation({
    mutationFn: async (rule: NotificationRule) => {
      const { data, error } = await supabase.rpc('push_retailer_notification', {
        p_rule_id: rule.id,
    ...
    onSuccess: (count) => {
      toast.success(`Notification sent to ${count} retailer(s)`);
    },
    onError: (err: any) => {
  ...
  const receiverLabel = (rule: NotificationRule) => {
    ...
    if (rule.receiver_type === 'retailer') {
      const target = rule.retailer_target_type || 'all';
      const count = rule.retailer_target_ids?.length || 0;
      if (target === 'all') return 'All Retailers';
      return `Retailers (${target}: ${count})`;
    }
      ...
      <div className="min-h-screen bg-gradient-subtle p-4">
        <div className="max-w-6xl mx-auto space-y-6">
          <div className="flex items-center gap-4">
            ...
            <Button onClick={() => setShowBroadcast(true)} variant="outline" className="gap-2">
              <Megaphone size={16} /> Push Notification
            </Button>
                    ...
                    {rules.map((rule) => (
                      <TableRow key={rule.id}>
                        ...
                        <TableCell className="text-right space-x-1">
                          {rule.receiver_type === 'retailer' && rule.is_active && (
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => pushMutation.mutate(rule)}
