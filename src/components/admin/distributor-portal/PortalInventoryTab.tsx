                  ...
                  return (
                    <TableRow key={item.id}>
                      ...
                      <TableCell className="text-right font-medium">
                        {(() => { const { displayQty, displayUnit } = getDisplayValues(item.quantity, 1, 1, item.unit || ''); return `${displayQty.toLocaleString('en-IN')}${displayUnit ? ` ${displayUnit}` : ''}`; })()}
                      </TableCell>
                      <TableCell className="text-right">
                        {(() => { const { displayQty, displayUnit } = getDisplayValues(item.available_quantity, 1, 1, item.unit || ''); return `${displayQty.toLocaleString('en-IN')}${displayUnit ? ` ${displayUnit}` : ''}`; })()}
                      </TableCell>
                      <TableCell>
