...
import { RetailerTargetingFields } from './RetailerTargetingFields';
...
const SOURCE_TABLES = [
  ...
  'retailers', 'attendance', 'branding_requests', 'product_schemes',
];
...
const RECEIVER_TYPES = [
  ...
  { value: 'retailer', label: 'Retailer (Customer Portal)' },
];
...
const BASE_PLACEHOLDERS = ['{user_name}', '{module_name}', '{record_name}', '{date}', '{points}'];
const RETAILER_PLACEHOLDERS = ['{scheme_name}', '{scheme_type}', '{retailer_name}'];
...
export function NotificationRuleForm({ rule, userId, onClose, onSaved }: NotificationRuleFormProps) {
  ...
  const [retailerTargetType, setRetailerTargetType] = useState(rule?.retailer_target_type || 'all');
  const [retailerTargetIds, setRetailerTargetIds] = useState<string[]>(rule?.retailer_target_ids || []);
  ...
  const placeholders = receiverType === 'retailer'
    ? [...BASE_PLACEHOLDERS, ...RETAILER_PLACEHOLDERS]
    : BASE_PLACEHOLDERS;
  ...
  const renderPreview = (template: string) => {
    return template
      ...
      .replace('{points}', '100')
      .replace('{scheme_name}', 'Summer Sale 2026')
      .replace('{scheme_type}', 'Discount')
      .replace('{retailer_name}', 'ABC Store');
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const payload: any = {
        ...
        retailer_target_type: receiverType === 'retailer' ? retailerTargetType : null,
        retailer_target_ids: receiverType === 'retailer' && retailerTargetType !== 'all' ? retailerTargetIds : null,
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Notify retailers on new scheme" />
          </div>
          <div className="space-y-2">
        ...
        {receiverType === 'retailer' && (
          <RetailerTargetingFields
            targetType={retailerTargetType}
            targetIds={retailerTargetIds}
            onTargetTypeChange={setRetailerTargetType}
            onTargetIdsChange={setRetailerTargetIds}
          />
        )}
