...
const TerritoriesManagement = () => {
  ...
  const [savingCustomArea, setSavingCustomArea] = useState(false);
  ...
  const areasSelected = selectedAreas.length > 0;

  // Load states via RPC (server-side DISTINCT - instant)
  const loadStates = useCallback(async () => {
    ...
    setAvailableStates((data || []).map((d: any) => d.statename).filter(Boolean));
    setLoadingStates(false);
  }, []);
  ...
  const loadDistrictsForState = useCallback(async (state: string) => {
    ...
    setAvailableAreas([]);
    const { data, error } = await supabase.rpc('get_distinct_districts', { p_state: state });
    if (error) {
      console.error('Error fetching districts:', error);
      setLoadingDistricts(false);
      return;
    }
    setAvailableDistricts((data || []).map((d: any) => d.district).filter(Boolean));
    setLoadingDistricts(false);
  }, []);

  const loadPincodesForDistrict = useCallback(async (state: string, district: string) => {
    setLoadingPincodes(true);
    setAvailablePincodes([]);
    setAvailableAreas([]);
    const { data, error } = await supabase.rpc('get_distinct_pincodes', { p_state: state, p_district: district });
    if (error) {
      console.error('Error fetching pincodes:', error);
      setLoadingPincodes(false);
      return;
    }
    setAvailablePincodes((data || []).map((d: any) => d.pincode).filter(Boolean));
    setLoadingPincodes(false);
  }, []);

  const loadAreasForPincode = useCallback(async (pincode: string) => {
    ...
    const { data } = await supabase
      ...
      .order('territory_po');
    if (data) {
      const unique = [...new Set(data.map(d => d.territory_po).filter(Boolean))] as string[];
      setAvailableAreas(unique);
    }
    setLoadingAreas(false);
  }, []);

  const saveCustomArea = useCallback(async () => {
    if (!customAreaName.trim() || !selectedPincode) return;
    setSavingCustomArea(true);
    ...
    const { error } = await supabase
      .from('pincode_master')
      .insert({
        officename: customAreaName.trim(),
        territory_po: customAreaName.trim(),
        pincode: selectedPincode,
        district: refRow?.district || selectedDistrict || null,
        statename: refRow?.statename || selectedState || null,
        is_custom_area: true,
      });

    if (error) {
      console.error('Error saving custom area:', error);
      toast.error('Failed to save custom area');
      setSavingCustomArea(false);
      return;
    }

    toast.success(`Area "${customAreaName.trim()}" added successfully`);
    const areaName = customAreaName.trim();
    setCustomAreaName('');
    setShowCustomArea(false);
    
    await loadAreasForPincode(selectedPincode);
    setSelectedAreas(prev => [...prev, areaName]);
    setSavingCustomArea(false);
  }, [customAreaName, selectedPincode, selectedDistrict, selectedState, loadAreasForPincode]);

  const checkDuplicateAreas = useCallback(async (pincode: string, areas: string[]) => {
    ...
    const { data: existing } = await supabase
      .from('territories')
      .select('id, name, territory_po_list, pincode_ranges')
      .contains('pincode_ranges', [pincode]);
    
    if (existing && existing.length > 0) {
      const conflicts = existing
        .filter(t => t.id !== editingTerritoryId)
        .filter(t => {
          const existingAreas = (t as any).territory_po_list || [];
          return areas.some(a => existingAreas.includes(a));
        });
      if (conflicts.length > 0) {
        const names = conflicts.map(c => c.name).join(', ');
        setDuplicateWarning(`Warning: Some selected areas are already mapped to: ${names}`);
      } else {
        setDuplicateWarning('');
      }
    } else {
  ...
  const handleAddTerritory = async (e: React.FormEvent) => {
    ...
    // Use selectedPincode if available, otherwise fall back to free-text
    const pincodeArray = selectedPincode 
      ? [selectedPincode] 
      : pincodes.split(',').map(p => p.trim()).filter(p => p.length > 0);

    if (selectedAreas.length === 0 && selectedPincode) {
      toast.error('Please select at least one area within the PIN code');
      return;
    }
    ...
    // Auto-suggest territory name if empty
    const finalTerritoryName = territoryName || (selectedAreas.length > 0 
      ? selectedAreas.slice(0, 2).join(' + ') + (selectedAreas.length > 2 ? ` +${selectedAreas.length - 2} more` : '') + ' Territory'
      : '');

    if (!finalTerritoryName) {
      toast.error('Please enter a territory name');
      return;
    }

    const territoryData = {
      name: finalTerritoryName,
      region,
      territory_type: territoryType || null,
      pincode_ranges: pincodeArray,
      territory_po_list: selectedAreas.length > 0 ? selectedAreas : null,
    ...
    if (editingTerritoryId) {
      const { error } = await supabase.from('territories').update(territoryData).eq('id', editingTerritoryId);
      if (error) {
      ...
      if (error) {
        ...
        toast.error('Failed to add territory: ' + error.message);
        return;
      }
  ...
  const handleEditTerritory = async (territory: any) => {
    ...
    // Populate PIN hierarchy from existing data
    const territoryPoList = (territory as any).territory_po_list || [];
    setSelectedAreas(territoryPoList);
    
    const pincode = territory.pincode_ranges?.[0] || '';
    if (pincode) {
      // Reverse lookup state/district from pincode_master
      ...
      if (pincodeData && pincodeData.length > 0) {
        ...
        const district = pincodeData[0].district || '';
        setSelectedState(state);
        setSelectedDistrict(district);
        setSelectedPincode(pincode);
        
        // Load cascading data
        if (state) await loadDistrictsForState(state);
        if (state && district) await loadPincodesForDistrict(state, district);
        if (pincode) await loadAreasForPincode(pincode);
      }
    }
              ...
              <form onSubmit={handleAddTerritory} className="space-y-6">
                ...
                <div className="space-y-4 p-4 border rounded-lg bg-muted/30">
                  <div className="flex items-center gap-2">
                    ...
                    <Label className="text-base font-semibold">Step 1: Select Location</Label>
                  </div>
                    ...
                    <div>
                      ...
                      <SearchableSelect
                        ...
                        onValueChange={(val) => {
                          ...
                          setAvailableAreas([]);
                          setDuplicateWarning('');
                          if (val) loadDistrictsForState(val);
                        }}
                    ...
                    <div>
                      ...
                      <SearchableSelect
                        ...
                        onValueChange={(val) => {
                          ...
                          setAvailableAreas([]);
                          setDuplicateWarning('');
                          if (val && selectedState) loadPincodesForDistrict(selectedState, val);
                        }}
                    ...
                    <div>
                      ...
                      <SearchableSelect
                        ...
                        onValueChange={(val) => {
                          ...
                          setAvailableAreas([]);
                          setDuplicateWarning('');
                          if (val) loadAreasForPincode(val);
                        }}
                ...
                {selectedPincode && (
                  <div className="space-y-4 p-4 border rounded-lg bg-muted/30">
                    <div className="flex items-center gap-2">
                      ...
                      <Label className="text-base font-semibold">Step 2: Select Areas within PIN Code {selectedPincode}</Label>
                    </div>
                    {loadingAreas ? (
                    ...
                    ) : availableAreas.length === 0 && !showCustomArea ? (
                      <p className="text-sm text-muted-foreground">No areas found for this PIN code. You can add a custom area below.</p>
                    ) : null}

                    {/* Blue banner + areas from PIN Code Master */}
                    {!loadingAreas && availableAreas.length > 0 && (
                      <>
                        <div className="px-3 py-1.5 bg-blue-50 border border-blue-200 rounded-md text-xs font-semibold text-blue-700 mb-2">
                          Areas from PIN Code Master
                        </div>
                          ...
                          <Checkbox
                            ...
                            onCheckedChange={(checked) => {
                              if (checked) {
                                ...
                                checkDuplicateAreas(selectedPincode, availableAreas);
                              } else {
                                setSelectedAreas([]);
                                setDuplicateWarning('');
                              }
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 max-h-48 overflow-y-auto">
                          {availableAreas.map((area) => (
                            <div key={area} className="flex items-center gap-2 p-2 rounded hover:bg-muted/50">
                              <Checkbox
                                ...
                                onCheckedChange={(checked) => {
                                  ...
                                  checkDuplicateAreas(selectedPincode, newAreas);
                                }}
                              />
                    ...
                    {/* Custom Area: Other (Specify area name) */}
                    {!loadingAreas && (
                      <div className="mt-3 space-y-2">
                        <div className="flex items-center gap-2">
                          <Checkbox
                            checked={showCustomArea}
                            onCheckedChange={(checked) => {
                              setShowCustomArea(!!checked);
                              if (!checked) setCustomAreaName('');
                            }}
                          />
                          <Label className="text-sm cursor-pointer font-medium">Other (Specify area name)</Label>
                        </div>
                        {showCustomArea && (
                          <div className="flex items-center gap-2 ml-6">
                            ...
                            <Button
                              ...
                              disabled={!customAreaName.trim() || savingCustomArea}
                            >
                              {savingCustomArea ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : 'Save'}
                            </Button>
                          </div>
                        )}
                      </div>
                    ...
                    {selectedAreas.length > 0 && (
                      <div className="mt-3 p-3 bg-primary/5 border border-primary/20 rounded-lg">
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="font-medium">PIN: {selectedPincode}</span>
                          <span className="text-muted-foreground">|</span>
                          <span>Areas: {selectedAreas.join(', ')} ({selectedAreas.length} selected)</span>
                        </div>
                      </div>
                    )}

                    {/* Duplicate Warning - Yellow theme */}
                    {duplicateWarning && (
                      <div className="flex items-center gap-2 p-3 bg-yellow-50 border border-yellow-300 rounded-lg text-sm text-yellow-800">
                ...
                {/* Section 3: Territory Details (enabled only after area selection) */}
                <div className={cn("space-y-4 p-4 border rounded-lg", areasSelected ? "bg-muted/30" : "bg-muted/10 opacity-60")}>
                  <div className="flex items-center gap-2">
                    <Sparkles className="h-4 w-4 text-primary" />
                    <Label className="text-base font-semibold">Step 3: Territory Details</Label>
                    {!areasSelected && (
                      <span className="text-xs text-muted-foreground ml-2">(Select areas first to enable)</span>
                    )}
                  </div>

                  <fieldset disabled={!areasSelected} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        />
                        {selectedAreas.length > 0 && !territoryName && (
                          <p className="text-xs text-muted-foreground mt-1">
                            Leave empty to auto-name: "{selectedAreas.slice(0, 2).join(' + ')}{selectedAreas.length > 2 ? ` +${selectedAreas.length - 2} more` : ''} Territory"
                          </p>
                        )}
                      </div>
                      <div>
                ...
                <div className="flex gap-2">
                  <Button type="submit" disabled={!areasSelected}>{editingTerritoryId ? 'Update' : 'Save'}</Button>
