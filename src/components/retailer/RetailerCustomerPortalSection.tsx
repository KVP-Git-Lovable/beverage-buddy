...
interface PortalData {
  ...
  lastPortalOrder: { id: string; order_number?: string; created_at: string; total_amount: number } | null;
...
export function RetailerCustomerPortalSection({ retailerId, retailerPhone, portalEnabled, portalPin, onPortalUpdate }: Props) {
  ...
  const loadPortalData = async () => {
    setLoading(true);
    try {
      // Fetch current portal status from the database (not stale props)
      ...
      const currentEnabled = retailerData?.portal_enabled ?? portalEnabled;
      const currentPin = retailerData?.portal_pin ?? portalPin;

      // Fetch portal orders count and last order
  ...
  const handleInitiatePortal = async () => {
    if (!retailerPhone) {
      toast({ title: "Phone Required", description: "Add a phone number to the retailer before enabling the portal.", variant: "destructive" });
      return;
    }
    setInitiating(true);
    try {
      ...
      const { error } = await supabase
        .from('retailers')
        .update({ portal_enabled: true, portal_pin: pin })
        .eq('id', retailerId);
  ...
  const handleDeactivatePortal = async () => {
    try {
      const { error } = await supabase
        .from('retailers')
        .update({ portal_enabled: false, portal_pin: null })
        .eq('id', retailerId);
      ...
      <CardContent className="p-3 space-y-3">
        {loading ? (
          ...
          <>
            ...
            {/* Portal Access Info (when active) */}
                  ...
                  {portalData.portalPin && (
                    <div>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={copyPin} className="text-muted-foreground hover:text-foreground transition-colors">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {!portalData.portalEnabled ? (
                <Button
                  ...
                  className="gap-1.5"
                >
                  {initiating ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Power className="h-3.5 w-3.5" />}
