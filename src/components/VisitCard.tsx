...
export const VisitCard = ({
  ...
  const canCheckInOut = hasPermission('visit_retailer_check_in', 'can_read');
    ...
    try {
      ...
      if (mergedOrders.length > 0) {
        ...
        allItems.forEach(it => {
          ...
          const { displayQty, displayUnit, displayRate } = getDisplayValues(qty, rate, originalRate, unit);
      ...
      <CardContent className="p-3 sm:p-4">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 mb-3">
          <div className="flex-1 min-w-0">
            ...
            {/* Visit Tracking Indicator - shows after first action (check-in) or for portal/phone orders */}
            {(() => {
              // Determine order source from today's orders
              const dominantOrderSource = ordersTodayList.length > 0 
                ? (ordersTodayList as any[]).find(o => o.order_source === 'portal_order')?.order_source 
                  || (ordersTodayList as any[]).find(o => o.order_source === 'voice_assistant')?.order_source 
                  || null
                : null;
                ...
                return (
                  <div className="mt-1">
                    <VisitTrackingIndicator
                      ...
                      orderSource={dominantOrderSource}
                    />
                  </div>
                );
              }
              // Show indicator for portal/phone orders even without check-in log
              if (dominantOrderSource && hasOrderToday) {
                return (
                  <div className="mt-1">
                    <VisitTrackingIndicator
                      locationStatus="location_unavailable"
                      checkInTime={(ordersTodayList as any[])[0]?.created_at || new Date().toISOString()}
                      distance={null}
                      onClick={() => {}}
                      orderSource={dominantOrderSource}
                    />
        ...
        <div className="space-y-2">
          ...
          <div className={`grid gap-1.5 sm:gap-2 ${!locationFeatureLoading && isLocationEnabled && canCheckInOut ? 'grid-cols-4' : 'grid-cols-3'}`}>
            {!locationFeatureLoading && isLocationEnabled && canCheckInOut && (
              <Button
                size="sm"
