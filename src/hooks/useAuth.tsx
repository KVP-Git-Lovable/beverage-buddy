...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const signIn = async (email: string, password: string, role?: 'admin' | 'user') => {
    await monitoring.trace('user_login_process', async () => {
      ...
      if (data.user) {
        // ENFORCE PORTAL SEPARATION (FAIL-CLOSED):
        // Field Sales login (/auth) must reject pure-distributor users.
        // 1) Try get_user_type RPC (authoritative).
        // 2) If RPC errors, fall back to manual check: distributor row exists AND no profile_id.
        const rejectAsDistributor = async () => {
          await supabase.auth.signOut();
          const msg = 'This account belongs to the Distributor Portal. Please log in at /distributor-portal/login';
          toast.error(msg, {
            action: {
              label: 'Go to Distributor Login',
              onClick: () => { window.location.href = '/distributor-portal/login'; },
            },
            duration: 8000,
          });
          throw new Error(msg);
        ...
        try {
          const { data: userType, error: userTypeError } = await supabase
            .rpc('get_user_type', { p_user_id: data.user.id });

          if (userTypeError) {
            // RPC failed — fail-closed manual fallback
            devError('get_user_type RPC failed — running manual fallback check:', userTypeError);
            const [{ data: distRow }, { data: upRow }] = await Promise.all([
              supabase
            ...
            const isDistributor = !!distRow;
            const hasFieldProfile = !!upRow?.profile_id;

            // Pure distributor (no field profile) → reject
            if (isDistributor && !hasFieldProfile) {
              await rejectAsDistributor();
            }
          } else if (userType === 'distributor') {
            await rejectAsDistributor();
          }
        } catch (rpcErr: any) {
          // Propagate our own rejection
          if (rpcErr?.message?.includes('Distributor Portal')) {
            throw rpcErr;
          }
          devError('Portal-separation check threw unexpectedly:', rpcErr);
        }
  ...
  const signOut = async () => {
    ...
    // Also clear distributor portal context to prevent leaked sessions
