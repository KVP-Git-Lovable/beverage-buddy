...
export const useActivityEvents = () => {
  ...
  const fetchEventById = useCallback(async (eventId: string): Promise<ActivityEvent | null> => {
    const { data, error } = await supabase
      .from('activity_events')
      .select('*')
      .eq('id', eventId)
      .maybeSingle();
    if (error) {
      console.error('[useActivityEvents] fetchEventById error:', error);
      return null;
    }
    return (data as unknown as ActivityEvent) || null;
  }, []);

  const markEventCompleted = async (eventId: string): Promise<boolean> => {
    const { error } = await supabase
      .from('activity_events')
      .update({ status: 'completed' } as any)
      .eq('id', eventId);
    if (error) {
      console.error('[useActivityEvents] markEventCompleted error:', error);
      return false;
    }
    // Invalidate cache entries so re-fetches get fresh data
    activityCache.clear();
    window.dispatchEvent(new CustomEvent('visitDataChanged'));
    return true;
  };

  const updateEventNotes = async (eventId: string, remarks: string): Promise<boolean> => {
    const { error } = await supabase
      .from('activity_events')
      .update({ remarks })
      .eq('id', eventId);
    if (error) {
      console.error('[useActivityEvents] updateEventNotes error:', error);
      return false;
    }
    activityCache.clear();
    return true;
