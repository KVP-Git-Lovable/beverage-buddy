...
serve(async (req) => {
  ...
  try {
    const { transcript, productNames, language } = await req.json();
    ...
    const langName = language === 'hi-IN' ? 'Hindi' : language === 'kn-IN' ? 'Kannada' : language === 'ml-IN' ? 'Malayalam' : language === 'ta-IN' ? 'Tamil' : 'English';
...
The user may speak in ${langName} or mix languages. Handle transliterated product names.
...
4. Handle ${langName}/English mixed inputs and transliterated names
5. If quantity is unclear, default to 1
6. If unit is unclear, default to "kg"
7. Convert any ${langName} number words to digits (e.g., "do" → 2, "paanch" → 5)
