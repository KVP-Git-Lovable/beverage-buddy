...
CREATE OR REPLACE FUNCTION public.get_product_units(p_product_id uuid)
 RETURNS TABLE(mapping_id uuid, uom_id uuid, code text, name text, category text, conversion_to_base numeric, is_base boolean, is_default_sales boolean, is_active boolean, is_price_basis boolean)
 LANGUAGE sql
...
AS $function$
  ...
  WHERE p.product_id = p_product_id
    ...
    AND (
      COALESCE(p.is_base, false) = true                 -- always keep the product base
      OR COALESCE(eu.enabled, true) = true              -- otherwise must be enabled in master
    )
