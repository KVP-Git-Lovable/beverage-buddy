CREATE OR REPLACE FUNCTION public.search_products_for_order(p_query text, p_category text DEFAULT NULL::text, p_limit integer DEFAULT 30, p_is_focused boolean DEFAULT NULL::boolean)
 RETURNS TABLE(id uuid, sku text, name text, rate numeric, unit text, closing_stock integer, is_active boolean, category_name text, is_focused_product boolean, default_uom_code text, allowed_uom_codes text[], variants jsonb)
 LANGUAGE sql
 STABLE
...
AS $function$
  ...
  fuzzy_matched AS (
    -- Trigram fallback: only used when the exact match returned nothing AND
    -- the user typed something. Catches small typos like "winolp" -> "WINOLAP".
    SELECT p.id
    FROM public.products p
    LEFT JOIN public.product_categories pc ON pc.id = p.category_id
    , q
    WHERE q.term <> ''
      AND NOT EXISTS (SELECT 1 FROM exact_matched)
      AND COALESCE(p.is_active, true) = true
      AND (p_category IS NULL OR p_category = 'all' OR pc.name = p_category)
      AND (p_is_focused IS NULL OR COALESCE(p.is_focused_product, false) = p_is_focused)
      AND (
        similarity(p.name, q.term) > 0.25
        OR similarity(p.sku, q.term) > 0.3
      )
    ORDER BY GREATEST(similarity(p.name, q.term), similarity(p.sku, q.term)) DESC
    LIMIT 50
  ),
  matched AS (
    SELECT id FROM exact_matched
    UNION
    SELECT id FROM fuzzy_matched
  ),
  uom_rows AS (
    SELECT
      pum.product_id,
  ...
  ORDER BY
    CASE WHEN q.term <> '' AND p.name ILIKE q.term || '%' THEN 0
         ...
         WHEN q.term <> '' AND (p.name ILIKE '%' || q.term || '%' OR p.sku ILIKE '%' || q.term || '%') THEN 2
         WHEN q.term <> '' THEN 3 - LEAST(GREATEST(similarity(p.name, q.term), similarity(p.sku, q.term)), 0.99)::numeric
         ELSE 4
    END,
    p.name
