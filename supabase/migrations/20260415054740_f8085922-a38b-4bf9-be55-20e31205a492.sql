-- Fix create_packing_list_atomic: quantity→available_qty, batch_number→batch_no, decrement available_qty on reserve
CREATE OR REPLACE FUNCTION public.create_packing_list_atomic(
  p_distributor_id uuid, p_delivery_date date, p_order_type text, p_warehouse_id uuid,
  p_total_value numeric, p_items jsonb, p_order_ids uuid[],
  p_batch_allocations jsonb DEFAULT NULL::jsonb, p_sources jsonb DEFAULT NULL::jsonb,
        ...
        IF (v_alloc->>'product_id') = (v_item->>'product_id') THEN
          IF v_alloc->'manual_batches' IS NOT NULL AND jsonb_array_length(v_alloc->'manual_batches') > 0 THEN
            ...
            LOOP
              ...
              IF v_batch.available_qty < v_alloc_qty THEN
                RAISE EXCEPTION 'Batch % insufficient stock: avail %, need %', v_batch.batch_no, v_batch.available_qty, v_alloc_qty;
              END IF;
              ...
              SET reserved_qty = reserved_qty + v_alloc_qty,
                  available_qty = available_qty - v_alloc_qty
              WHERE id = v_batch.id;
          ...
          ELSE
            ...
            LOOP
              ...
              v_alloc_qty := LEAST(v_remaining, v_batch.available_qty);
...
-- Fix delete_packing_list_atomic: restore available_qty when releasing
...
BEGIN
  ...
  LOOP
    ...
    SET reserved_qty = GREATEST(0, reserved_qty - v_batch.allocated_qty),
        available_qty = available_qty + v_batch.allocated_qty
...
-- Fix dispatch_packing_list_atomic: quantity→available_qty for deduction
...
BEGIN
  ...
  LOOP
    ...
    -- Deduct dispatched from reserved_qty; leftover goes back to available
    UPDATE inventory_batches
    SET reserved_qty = GREATEST(0, reserved_qty - v_batch.allocated_qty)
    WHERE id = v_batch.batch_id;
    -- Note: available_qty was already decremented during creation, no change needed for dispatched portion
    -- But leftover (unpicked) needs to be restored to available_qty
    IF v_leftover > 0 THEN
      UPDATE inventory_batches
      SET available_qty = available_qty + v_leftover
      WHERE id = v_batch.batch_id;
    END IF;
  ...
  IF v_pl.distributor_id IS NOT NULL THEN
    UPDATE distributor_inventory di
    SET quantity = COALESCE((SELECT SUM(ib.available_qty + ib.reserved_qty) FROM inventory_batches ib WHERE ib.distributor_id = di.distributor_id AND ib.product_id = di.product_id AND (di.warehouse_id IS NULL OR ib.warehouse_id = di.warehouse_id)), 0),
  ...
  -- Backorder handling from sources
  FOR v_source IN
