...
BEGIN
  ...
  CASE p_action
    WHEN 'OPENING_STOCK' THEN
      IF v_inventory.id IS NULL THEN
        ...
        SELECT
          p_distributor_id, p_product_id, COALESCE(p.name, 'Product'), p_quantity,
          0, 0, 0,
      ...
      IF p_batch_no IS NOT NULL AND p_batch_no <> '' THEN
        INSERT INTO public.inventory_batches (
          distributor_id, product_id, batch_no, expiry_date, available_qty, warehouse_id
        ) VALUES (
          p_distributor_id, p_product_id, p_batch_no, p_expiry_date, p_quantity, v_wh_id
        )
  ...
  -- Fixed: use 'batch_number' instead of 'batch_id'
  INSERT INTO public.distributor_inventory_transactions (
    ...
    p_notes, p_created_by, p_batch_no, NULL, p_reference_id,
